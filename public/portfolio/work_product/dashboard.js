d3_4.request("/portfolio/wp-investor-query").response(function (xhr) {return xhr.responseText}).get(function(data) {
...
function draw(data){
    ...
    function formatsData(data){
        ...
        data.forEach(function(d,i){
            ...
            d["tags"] = wordFreq(d["tags"]);
            d["funding_round_code"] = wordFreq(d["funding_round_code"]);
            d["id_vis"] = i;
        });
        return data
    };
    ...
    function table(xf, data, selector){
        ...
        $('#new thead tr').clone(true).appendTo('#new thead');
        $('#new thead tr:eq(1) th').each( function (i) {
            var title = $(this).text();
    ...
    function timeSeries(data, selector){
        ...
        function vals(d){
            var vec = d["raised_amount_usd"].split(":")
            var arrayLength = vec.length;
        ...
        function months(d){
            var vec = d["funded at"].split(":")

            var arrayLength = vec.length;
            for (var i = 0; i < arrayLength; i++) {
                vec[i] = vec[i].slice(0, -3)
            }
            return vec
        }
    ...
    function draw2(investor_data){
        timeSeries(investor_data, "#metric-modal")
    }
